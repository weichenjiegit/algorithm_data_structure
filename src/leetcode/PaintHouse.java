...
public class PaintHouse {
	...
	public int minCost2(int[][] costs) {
		...
		int n = costs.length, currentR = 0, currentG = 0, currentB = 0;
		for (int i = 0; i < n; i++) {
			int lastR = currentR, lastB = currentB, lastG = currentG;
			currentR = costs[i][0] + Math.min(lastB, lastG);
			currentB = costs[i][1] + Math.min(lastR, lastG);
			currentG = costs[i][2] + Math.min(lastR, lastB);
		}
