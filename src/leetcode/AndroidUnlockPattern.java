 ...
 * other keys must have previously selected in the pattern. No jumps through non selected key is
 * allowed.
 * 
...
public class AndroidUnlockPattern {
	...
	private int DFS(boolean vis[], int[][] skip, int cur, int remain) {
		...
		int rst = 0;
		for (int i = 1; i <= 9; ++i) {
			// If vis[i] is not visited and (two numbers are adjacent or skip number is already
			// visited)
