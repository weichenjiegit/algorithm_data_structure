...
public class RemoveDuplicateLetters {
	public String removeDuplicateLetters(String s) {
		...
		for (int i = 0; i < s.length(); i++)
			count[s.charAt(i) - 'a']++;
		for (int i = 0; i < s.length(); i++) {
			if (s.charAt(i) < s.charAt(pos)) // small means high lexicographical order
				pos = i;
			count[s.charAt(i) - 'a']--;
	...
	public String removeDuplicateLetters2(String sr) {

		int[] res = new int[26]; // will contain number of occurences of character (i+'a')
		boolean[] visited = new boolean[26]; // will contain if character (i+'a') is present in
		...
		for (char s : ch) {
			...
			// the string again
			// it can be removed and added later e.g stack = bc remaining string abc then a can pop
			// b and then c
